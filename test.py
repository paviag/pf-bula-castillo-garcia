...
model_path = "runs/detect/train63/weights/best.pt"
...
for r in pos_results:
    if len(r.boxes.cls.unique()) > 1:
        raise Exception(r.boxes.cls)

ytrue = np.concatenate([
    np.zeros(len(neg_results)),
    np.ones(len(pos_results)),
])
ypred = np.array([len(r.boxes.cls.unique()) for r in neg_results+pos_results])

ConfusionMatrixDisplay.from_predictions(ytrue, ypred, cmap='Blues')
plt.show()
